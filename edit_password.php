...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    $newPassword = $_POST['new_password'];  // Password baru dari form
    ...
    if ($API) {
        ...
        if (count($user) > 0) {
            // Jika user ditemukan, lakukan update password
            $API->write('/ip/hotspot/user/set', false);
            $API->write('=.id=' . $user[0]['.id'], false);
            $API->write('=password=' . $newPassword, true);  // Update password
            $API->read();

            $message = "Password untuk user '{$userHotspot}' berhasil diubah!";
        } else {
                ...
                <div class="row">
                    <div class="col-lg-12">
                        <h1 class="page-header">
                            Edit Password Untuk User : <?php echo $userHotspot; ?>
                        </h1>
                        <ol class="breadcrumb">
                            <li>
